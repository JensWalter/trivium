 ...
 */
package io.trivium.dep.org.iq80.leveldb.util;
            ...
            implements SPI
    {
                ...
                throws IOException
        {
            ...
            if (uncompressed.hasArray()) {
                ...
                int t = io.trivium.dep.org.iq80.snappy.Snappy.getUncompressedLength(input, inputOffset);
                output = new byte[t];
                outputOffset = 0;
            }

            int count = io.trivium.dep.org.iq80.snappy.Snappy.uncompress(input, inputOffset, length, output, outputOffset);
            if (uncompressed.hasArray()) {
        ...
        public int uncompress(byte[] input, int inputOffset, int length, byte[] output, int outputOffset)
                throws IOException
        {
            return io.trivium.dep.org.iq80.snappy.Snappy.uncompress(input, inputOffset, length, output, outputOffset);
        }
        ...
        public int compress(byte[] input, int inputOffset, int length, byte[] output, int outputOffset)
                throws IOException
        {
            return io.trivium.dep.org.iq80.snappy.Snappy.compress(input, inputOffset, length, output, outputOffset);
        }
        ...
        {
            return io.trivium.dep.org.iq80.snappy.Snappy.maxCompressedLength(length);
        }
    }
        ...
        for (int i = 0; i < factories.length && attempt == null; i++) {
            ...
            try {
                ...
                if ("xerial".equals(name.toLowerCase())) {
                    name = "io.trivium.dep.org.iq80.leveldb.util.Snappy$XerialSnappy";
                }
                else if ("iq80".equals(name.toLowerCase())) {
                    name = "io.trivium.dep.org.iq80.leveldb.util.Snappy$IQ80Snappy";
                }
