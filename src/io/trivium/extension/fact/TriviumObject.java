...
public class TriviumObject implements Fact {
    ...
    public void setTypeRef(TypeRef newType){
        typeRef = newType;
    ...
    public void addMetadata(String name,String value){
        ...
        if(name.equals("typeId")){
            setTypeRef(TypeRef.getInstance(value));
        }else
    ...
    public <T extends Fact> T getTypedData(){
        TypeRef typeRef = this.getTypeRef();
        try {
            Class<? extends Fact> clazz = Registry.INSTANCE.types.get(typeRef);
            if(clazz==null){
                //no registered type found
                return (T) this;
            }else {
                //build from factory
                T obj = (T) Registry.INSTANCE.types.get(typeRef).newInstance();
                obj.populate(this);
                return obj;
            }
        }catch(Exception ex){
    ...
    private void checkMetadata() {
        ...
        if (typeRef == null || typeRef == TypeRef.INVALID) {
            if (metadata!=null && metadata.hasKey("typeId")) {
                typeRef = TypeRef.getInstance(metadata.findValue("typeId"));
            }
        }
        ...
        if (!meta.hasKey("typeId")) {
            meta.add(new NVPair("typeId", getTypeRef().toString()));
        }
