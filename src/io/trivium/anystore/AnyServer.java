...
public class AnyServer implements Runnable {
    ...
    public Result loadObjects(Query query) {
        ...
        try {
            ...
            for (ObjectRef ref : refs) {
                ...
                Class<?> typeClass = Registry.INSTANCE.types.get(tvm.getTypeRef());

                if (typeClass != null && query.targetType != TriviumObject.class
                        && !typeClass.equals(query.targetType)) {
                    continue;
                }
                Fact fact;
    ...
    public TriviumObject loadObjectById(ObjectRef key) {
        ...
        po.setTypeRef(TypeRef.getInstance(meta.findValue("typeId")));
